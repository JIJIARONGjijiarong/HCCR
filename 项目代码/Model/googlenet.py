# -*- coding: utf-8 -*-
"""
Created on Tue Nov 12 10:01:23 2019
...
# 设置一些参数(如批量大小、图像尺寸、反向传播迭代次数、最后所分类别、所选函数、存储目录等)
...
parse.add_argument('--log_path', type=str, default='E:\Git\HCCR\data\log\googlenet_log_100.pth', help='dir of checkpoints')
...
parse.add_argument('--image_size', type=int, default=299, help='resize image')
parse.add_argument('--epoch', type=int, default=10)
parse.add_argument('--num_class', type=int, default=62, choices=range(10, 3755))
...
def train():
    ...
    model = inception_v3(pretrained=False, num_classes=args.num_class, aux_logits=False)
    ...
    while epoch < args.epoch:
        ...
        for i, data in enumerate(train_loader):
            ...
            sys.stdout.write(
                "time %5ds,epoch: %5d,batch: %5d,loss: %f  \r" % (end - start, epoch + 1, i + 1, running_loss))
            sys.stdout.flush()
            if i % 100 == 0:
                print('Save checkpoint:%5d,loss: %f' % (epoch, running_loss))
...
def validation():
    ...
    print('data size:',test_set.__len__())
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = inception_v3(pretrained=False, num_classes=args.num_class, aux_logits=False)
    model.to(device)
...
def inference():
    ...
    model = inception_v3(pretrained=False, num_classes=100, aux_logits=False)
    model.eval()  # 模型评价
    checkpoint = torch.load(args.log_path)
    model.load_state_dict(checkpoint['model_state_dict'])
    output = model(input)
...
# 将各个训练图像、测试图像的路径分别汇集存储到train.txt和test.txt中
